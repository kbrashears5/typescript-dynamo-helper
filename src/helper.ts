...
export class DynamoHelper extends BaseClass implements IDynamoHelper {
  ...
  public async BatchWriteAsync(
    ...
    if (this.ObjectOperations.IsNullOrEmpty(batchRequest)) {
      throw new Error(`[${action}]-Must supply batchRequest`);
    }
   ...
   */
  public async DeleteItemByKeyAsync(
    tableName: string,
    keyName: string,
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(tableName)) {
      throw new Error(`[${action}]-Must supply tableName`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(keyName)) {
      throw new Error(`[${action}]-Must supply keyName`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(keyValue.toString())) {
      throw new Error(`[${action}]-Must supply keyValue`);
    }
   ...
   * @param keyValue {string | number} Value of key column
   */
  public async GetItemByKeyAsync(
    tableName: string,
    keyName: string,
    keyValue: string | number,
   ...
   */
  public async PutItemByKeyAsync<T extends AttributeValue>(
    tableName: string,
    item: T,
  ): Promise<DynamoDB.GetItemOutput> {
    ...
    if (!item || Object.keys(item).length === 0) {
      throw new Error(`[${action}]-Must supply item`);
    }
  ...
  public async ScanAsync(
    ...
    if (this.ObjectOperations.IsNullOrEmpty(attributeNames)) {
      throw new Error(`[${action}]-Must supply attributeNames`);
    }
    if (this.ObjectOperations.IsNullOrEmpty(attributeValues)) {
      throw new Error(`[${action}]-Must supply attributeValues`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(expression)) {
      throw new Error(`[${action}]-Must supply expression`);
    }
    ...
    if (this.ObjectOperations.IsNullOrEmpty(lastEvaluatedKey)) {
    ...
    if (response.LastEvaluatedKey && response.Items) {
      const nextParameters = await this.ScanAsync(
        tableName,
        attributeNames,
        attributeValues,
        expression,
  ...
  public async UpdateByKeyAsync(
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(conditionExpression)) {
      throw new Error(`[${action}]-Must supply conditionExpression`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(updateExpression)) {
      throw new Error(`[${action}]-Must supply updateExpression`);
    }
